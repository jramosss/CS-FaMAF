...
int main(int argc, char *argv[]) {
    ...
    max_temp_anual(array, output);
    printf(" \n La temperatura minima historica es: %d \n", h_min_temp(array));
      for (unsigned int i = 0; i<YEARS; i++){
      printf("La temperatura maxima de el año %u fue: %d \n", i+FST_YEAR, output[i]);
    }

    max_yearly_rain (array, registro);
      for (unsigned int j = 0; j<YEARS; j++ ){
      printf("El año %u tuvo su mayo precipitacion en el mes Nº %d \n",j+FST_YEAR, registro[j]);
    }
    return (EXIT_SUCCESS); 
}
