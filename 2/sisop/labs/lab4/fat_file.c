 ...
 */
fat_dir_entry fat_file_init_empty_direntry(bool is_dir, char *filepath,
                                           u32 start_cluster) {
    ...
    new_entry->create_time_fine_res = 0;         // No idea what this is
...
fat_file fat_file_init(fat_table table, bool is_dir, char *filepath) {
    ...
    new_file->dentry =
        fat_file_init_empty_direntry(is_dir, filepath, start_cluster);
...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    /* ####################### */
    if (disk_dentry->base_name[0] == 0xe5 &&
...
void fat_file_truncate(fat_file file, off_t offset, fat_file parent) {
    ...
    last_cluster = fat_table_get_next_cluster(file->table, last_cluster);
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    u32 cluster = 0,next_free_cluster = 0;
    ...
    if (file->dentry->file_size < size + offset){
    ...
    while (bytes_remaining > 0) {
        ...
        if (bytes_remaining < 512) {
            ...
            fat_table_set_next_cluster(file->table,next_free_cluster,FAT_CLUSTER_END_OF_CHAIN); // SI QUEDAN MENOS; SETEO EOC y se rompe
    ...
    //printf("FAT TABLE DESPUES DE ESCRIBIR\n");
    //fat_table_print(file->table,0,61);
