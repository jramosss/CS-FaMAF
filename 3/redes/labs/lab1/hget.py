...
def connect_to_server(server_name):
    ...
    """
    """
    Obtengo la direccion ip de la pagina con el dominio indicado por {server_name}
    La funcion getaddrinfo retorna un valor de tipo: 
    * List[Tuple[AddressFamily,SocketKind],int,str,Tuple[str,int]
    donde el index [0][4] es la tupla (direccion_ip,port) por lo que 
    indexada en [0][4][0] tenemos el ip_port
    """

    ip_address = str(socket.getaddrinfo(server_name,HTTP_PORT)[0][4][0])
    sys.stderr.write("Contactando al servidor en %s...\n" % ip_address)
    
    #Creo una conexion con el puerto especificado
    return socket.create_connection((server_name,HTTP_PORT))
